...
# ===============================
# Hard-coded Fandom configuration
# ===============================
BASE = "/home/sundeep/Fandom-Span-Identification-and-Retrieval"

# Paths
EMBEDDINGS_PATH = f"{BASE}/2.Embeddings/embeddings"             # Pickled dict {(article_id, paragraph_id): np.array(384,)}
MASTER_CSV      = f"{BASE}/master_csv.csv"                      # Final master with article_id, paragraph_id, paragraph_text
QUERIES_CSV     = f"{BASE}/1.Fandom_Dataset_Collection/query/queries.csv"
TITLE_TO_ID_JSON= f"{BASE}/title_to_id_mapping.json"

# Outputs
OUT_DIR         = f"{BASE}/4.Retrieval"
os.makedirs(OUT_DIR, exist_ok=True)
OUTPUT_LOG      = f"{OUT_DIR}/retrieval_log.txt"
RETRIEVED_DOCS  = f"{OUT_DIR}/retrieved_docs_L6.csv"
QUERY_DOC_SCORES= f"{OUT_DIR}/query_doc_scores_L6.csv"
SUMMARY_METRICS = f"{OUT_DIR}/retrieval_metrics_L6.csv"
...
if __name__ == "__main__":
    ...
    # Load title->id mapping
    with open(TITLE_TO_ID_JSON, 'r') as f:
        title_to_id_mapping = json.load(f)
    logging.info("Loaded title_to_id_mapping.json")
