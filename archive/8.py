...
# --- Phase 1: Build a master ID mapping (numerical ID to filename string) ---
def build_master_id_map(links_dir):
    id_to_title_map = {}
    if not os.path.isdir(links_dir): return id_to_title_map
    ...
    for fn in csv_files:
        ...
        try:
            with open(path, "r", encoding="utf-8", newline="") as f:
                ...
                if not article_id_col: continue

                first_row = next(reader, None)
                if first_row and first_row.get(article_id_col):
                    numerical_id = int(first_row[article_id_col])
                    id_to_title_map[numerical_id] = title
        except Exception:
            continue
    return id_to_title_map

# --- Phase 2: Load and unify link data using the master map ---
def load_and_unify_links(links_dir, id_to_title_map):
    per_para = {}
    per_article = {}
            ...
            with open(path, "r", encoding="utf-8", newline="") as f:
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        unified_link_ids = [id_to_title_map.get(num_id, "unknown") for num_id in numerical_link_ids]
                        unified_links = (links_str, unified_link_ids)
                    except (ValueError, SyntaxError):
                        unified_links = (links_str, [])

                    pid_val = (row.get(para_col) or "").strip() if para_col else ""
                    if pid_val != "":
                        try:
                            pid = int(pid_val)
                            per_para[(current_article_title, pid)] = unified_links
                            saw_para_row = True
                        except Exception: pass
                    else:
                        fallback_article_links = unified_links
...
def main(plaintext_dir, html_dir, output_csv_path, links_dir_path):
    ...
    # Step 1: Build the master mapping
    print("[1/3] Building master ID mapping...", flush=True)
    id_to_title_map = build_master_id_map(links_dir_path)
    if not id_to_title_map:
        print("[fatal] Master mapping could not be built. Check CSVs.", flush=True); sys.exit(1)
    
    # Step 2: Load and unify link data
    print("[2/3] Loading and unifying link data...", flush=True)
    links_per_para, links_per_article = load_and_unify_links(links_dir_path, id_to_title_map)
    ...
    with open(output_csv_path, "w", encoding="utf-8", newline="") as out:
        ...
        for idx, fname in enumerate(files, 1):
            ...
            if not paras:
                ...
                writer.writerow({"article_id": article_id, "article_title": title, "paragraph_id": 0, "paragraph_text": "", "internal_links": links, "article_id_of_internal_link": link_ids})
            else:
                for p_id, para in enumerate(paras, 1):
                    links, link_ids = links_per_para.get((article_id, p_id), links_per_article.get(article_id, ("[]", "[]")))
